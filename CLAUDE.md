...
- Ett Views-fält byggt på `BulkForm` (t.ex. draggableviews) tvingar HELA displayen in i ett `<form>` så fort fältet finns i fältlistan, oavsett `exclude`-inställning - bryter varje CSS-grid/flex-layout som förväntar sig `.views-row` som direkt barn till `.view-content` (formuläret hamnar emellan). Sådana fält hör hemma på en dedikerad tabellstil-display, aldrig på ett kortgrid.
- `Url::fromRoute(...)->toString()` för en rutt med `_csrf_token: 'TRUE'` ger en cache-platshållare (`Crypt::hashBase64($path)`), INTE den riktiga sessionsbundna token - fungerar bara om värdet går genom Twig/render-pipelinens senare platshållarersättning. Ska token bäddas in direkt i `drupalSettings` eller liknande (utanför den vanliga rendercykeln): anropa `\Drupal::csrfToken()->get($path)` direkt, där `$path` är ruttens definierade sökväg utan inledande snedstreck.
- En ny Views-display som bara skriver till `display_options.filters`/`sorts`/`footer`/`empty`/`title`/`path` utan att också sätta motsvarande nyckel `false` i `display_options.defaults` ärver TYST huvuddisplayens värde för den sektionen, oavsett vad som faktiskt står lagrat i den nya displayens egen config. Syns bara i renderad HTML, inte vid config-inspektion (den lagrade configen ser rätt ut). Lös genom att explicit lista varje överskriven sektion i `defaults` som `false`.
...
- Innehållsmigreringen mot produktionen genomgången 2026-07-19: dirigenthistorik (9), info-korten (6), avdubblade låttips (56), menylänkarna, höstens 20 kalendarieposter och flaggmigreringen (878 besked - fler än de 853 migrerade, sajten är redan i skarp drift med riktiga medlemssvar) fanns alla redan på plats sedan den tidigare cutovern. Kyrkans namnbyte (`_import/byt-kyrknamn.php`) saknades och kördes nu - 10 kalendarieposters infotext rättade, verifierat med skiftlägeskänslig kontroll (kvarvarande gemena `betelkyrkan`-träffar är webbadresser, medvetet orörda). **Kvar:** lösenordsnollställningen - medvetet inte gjord, hör ihop med det faktiska lanseringsbeskedet till kören, inte en teknisk migrering.
- Fika-självanmälan 2026-07-19: ny modul `gob_fika` (`web/modules/custom/gob_fika/`), spegling av `gob_attendance`-mönstret men med `field_fikafixare` (befintligt flervärdesfält) som datalager istället för egen tabell. Rutt `/fika/{node}/{ja|nej}` tar aldrig en användarparameter - man kan bara lägga till/ta bort sig själv, per konstruktion. Knapparna "Jag tar fikat"/"Ta bort mig" i kalendariets utfällda kort, gamla "kontakta Ingierd"-texten borttagen.
- Ajax-uppdatering av kalendariekorten 2026-07-19: alla svarsknappar (närvaro + fika) uppdaterar sitt kort på plats via `attendance-ajax.js` (fetch + DOM-utbyte på `data-nid`) - ingen omladdning, scrollläge och utfällt/hopfällt läge bevaras. Utan JS eller vid fel faller länkarna tillbaka till vanlig helsideladdning.
- Inloggnings-/konto-/lösenordssidorna omdesignade 2026-07-19: `auth.css` + en rutt-styrd `page-auth`-bodyklass (`gob_preprocess_html`) ger dem samma kortstil som resten av sajten, med centrerade pillknappar för Logga in/Skapa nytt konto/Återställ ditt lösenord (aktiv = fylld lila). Avslöjade en äkta bugg i grundtemat: text-inputs saknade `width`/`box-sizing` och sizeade sig efter HTML-attributet `size` (t.ex. `size="60"` ≈ 448px), vilket bara inte syntes förut eftersom övriga formulär ligger i den breda 1040px-layouten - fixat i `base.css`, gäller nu alla formulär på sajten. Samtidigt flyttades huvudmenyn (`.menu--main`) att börja direkt efter loggan istället för att centrera sig i den lediga ytan.
- Bruksanvisning för körmedlemmarna 2026-07-19: PDF med sajtens designtoken-palett och Nunito-rubriker, byggd från `manual/manual.html` (gitignorerad arbetskatalog, regenereras med Playwright-skript som loggar in som testmedlem och skärmdumpar varje funktion). Ett kapitel = en sidstart för enkel bläddring. Täcker inloggning/lösenord, kalendariet, fikat, utskick, info (flyttad till den publika delen - `/info` är medvetet öppen för alla), matrikeln, egna uppgifter, notarkivet/stämmixern. Låttips-kapitlet borttaget (admin-funktion, inte för medlemmar). Måste laddas upp direkt på produktionssajten för att nå medlemmarna - filuppladdningar i DDEV är gitignorerade och följer aldrig med pipelinen.
- Tidigare händelser på kalendariesidan, driftsatt 2026-09-01: ny Views-display `page_2` på samma vy `kalendarium` (`/kalendarium/tidigare`), samma åtkomst som huvudsidan men omvänt datumfilter (`< Today` istället för `>= Today`) och fallande sortering (senaste passerade händelsen överst). Egen radmall `views-view-fields--kalendarium--page-2.html.twig` utan anmälningsknappar - bara den slutgiltiga statuschipen visas (Anmäld/Avanmäld/Ej svarat), fikalistan visas som ren historik. Länkar i båda riktningarna via vyns sidfot. Krävde en explicit `defaults`-karta i displayens config (`filters`/`sorts`/`footer`/`empty`/`title`/`path` satta `false`) - annars ärvs huvuddisplayens värden tyst trots att den nya displayens egna config är korrekt ifylld (se fallgroparna). Ren config-ändring, ingen innehållsmigrering - verifierat 132 tidigare + 19 kommande = databasens 151 kalendarieposter, huvudsidans knappar och filter opåverkade, anonym nekas (403) på båda sidorna live.
- Närvarosidan (`/gob/narvaro`, adminverktyg bakom behörigheten "access gob attending") 2026-09-01: samma hopfällbara kortstil som kalendariet, `gob/kalendarium`-biblioteket återanvänt rakt av (ingen ny CSS behövdes). Sammanfattningsrad under titeln ("Kommer: X, Kommer inte: Y, Saknar besked: Z") ersätter de gamla rubrikerna, listorna grupperade per stämma med globalt + lokalt löpnummer bevarat. Ny sida `/gob/narvaro/tidigare` (samma mönster/länkar som kalendariets), och gränsen för "kommande" ändrad från den gamla släppgränsen på -3 dagar till samma "idag"-gräns som kalendariet, för konsekvens mellan sidorna. Ren kod-/malländring i `gob_attending`-modulen, `ddev drush config:export` visade noll diff.
...
- Info-korten (2026-07-18): field_icon utökat till alla 1748 Lucide-ikoner (fältkonfig i config/sync, spriten i `web/themes/custom/gob/icons/lucide-sprite.svg`). De sex kortens ikonvärden rättade till riktiga Lucide-namn - skriptet `_import/info-alla-ikoner.php` kan köras om mot produktionsdatabasen.
- Svenska gränssnittsöversättningar (2026-07-19): fyra kärnsträngar låg med engelsk text sparad som "anpassad" svensk översättning (Edit, Email address, lösenordsåterställningstexterna) - locale:update rör aldrig anpassade rader. Rättade lokalt; skriptet `_import/svenska-oversattningar.php` kan köras om mot produktionsdatabasen (mail_login-texterna är däremot config och följer med git).
- Info-kortens ordning (2026-07-18): `drupal/draggableviews` provades men slog sönder kortgridet (se fallgroparna) - ersatt med eget fält `field_weight` (config i config/sync) + egen modul `gob_infokort` för att dra om korten direkt på /info. Skriptet `_import/info-viktfalt.php` satte startvärden 0/10/20... i dagens ordning - kan köras om mot produktionsdatabasen, men den faktiska dragna ordningen (om admin ändrat den lokalt) är bara lokal databasdata.
